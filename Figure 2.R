...
cm1c <- glmmTMB(feeding.rate.per.colony ~ batch, data = fr.c)

anova(cm1, cm1a) # No interaction between temperature and batch on colony-level clearance rate (χ2 = 2.799, df = 1, p = 0.094)
anova(cm1a, cm1b) # Batches have different intercepts (χ2 = 6.495, df = 1, p = 0.011)
anova(cm1a, cm1c) # Significant effect of temperature on colony-level clearance rate (χ2 = 4.779, df = 1, p = 0.029)

# Use confint() to compute confidence intervals for parameters for best fit model
confint(cm1a)
# The colony-level clearance rate increased by by 7.265 (1.232 – 13.298, 95% CI) ml per day for every °C increase in realized temperature
# The colony-level clearance rate was 86.124 (26.485 – 145.764, 95% CI) ml per day lower in batch 2 compared to batch 1.
...
zm1c <- glmmTMB(feeding.rate.per.zooid ~ + batch, data = fr.z)

anova(zm1, zm1a) # No interaction between temperature and batch (χ2 = 0.225, df = 1, p = 0.635)
anova(zm1a, zm1b) # Batches have the different intercepts (χ2 = 4.156, df = 1, p = 0.041)
anova(zm1a, zm1c) # Significant effect of temperature on clearance rate per zooid (χ2 = 6.41, df = 1, p = 0.011)

# Use confint() to compute confidence intervals for parameters for best fit model
confint(zm1a)
# The clearance rate per zooid increased by 0.062 (0.019 – 0.105, 95% CI) ml per day for every °C increase in realized temperature
# The clearance rate per zooid was 0.476 (0.048 – 0.904, 95% CI) ml per day lower in batch 2 compared to batch 1.
...
fm1c <- glmmTMB(feeding.rate.per.feedingzooid ~ batch, data = fr.f)

anova(fm1, fm1a) # No significant interaction between temperature and batch (χ2 = 0.005, df = 1, p = 0.943)
anova(fm1a, fm1b) # No difference in intercept between batches (χ2 = 0.894, df = 1, p = 0.345)
anova(fm1a, fm1c) # Significant effect of temperature on clearance rate per feeding zooid (χ2 = 9.777, df = 1, p = 0.002)

# Use confint() to compute confidence intervals for parameters for best fit model
confint(fm1a)
# The clearance rate per feeding zooid increased by by 0.323 (0.151 – 0.494, 95% CI) ml per day for every °C increase in realized temperature
...
text(x = seq(tick_positions[1], tick_positions[8], by = 2),
     ...
     cex = cex.axis,
     adj = 1)
with(p.c[p.c$batch == 1, ], polygon(c(realized.temp, rev(realized.temp)), c(lwr, rev(upr)), col = adjustcolor("tomato", alpha = 0.4), border = NA))
with(p.c[p.c$batch == 2, ], polygon(c(realized.temp, rev(realized.temp)), c(lwr, rev(upr)), col = adjustcolor("dodgerblue", alpha = 0.4), border = NA))
with(fr.c[fr.c$batch == 1, ], points(realized.temp,feeding.rate.per.colony, pch  = 16, cex = p.cex, col = adjustcolor("tomato", alpha = 0.4)))
...
text(x = seq(tick_positions[1], tick_positions[8], by = 2),
     ...
     cex = cex.axis,
     adj = 1)
with(p.z[p.z$batch == 1, ], polygon(c(realized.temp, rev(realized.temp)), c(lwr, rev(upr)), col = adjustcolor("tomato", alpha = 0.4), border = NA))
with(p.z[p.z$batch == 2, ], polygon(c(realized.temp, rev(realized.temp)), c(lwr, rev(upr)), col = adjustcolor("dodgerblue", alpha = 0.4), border = NA))
with(fr.z[fr.z$batch == 1, ], points(realized.temp, feeding.rate.per.zooid, pch  = 16, cex = p.cex, col = adjustcolor("tomato", alpha = 0.4)))
...
with(fr.f[fr.f$batch == 1, ], points(realized.temp, feeding.rate.per.feedingzooid, pch  = 16, cex = p.cex, col = adjustcolor("tomato", alpha = 0.4)))
